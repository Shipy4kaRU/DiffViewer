...
function convertToLinesOfEdits(
  linesOfDiffs: Diff[][],
  startLineNumber: number
) {
  return flatMap(linesOfDiffs, (diffs, i) =>
    diffsToEdits(diffs, startLineNumber + i)
  );
}
...
function diffChangeBlock(
  ...
  const oldEdits = convertToLinesOfEdits(
    splitDiffToLines(oldDiffs),
    oldStartLineNumber
  );
  const newEdits = convertToLinesOfEdits(
    splitDiffToLines(newDiffs),
    newStartLineNumber
  );
...
function diffByLine(
  ...
  const [oldEdits, newEdits] = changes.reduce<
    [RangeTokenNode[], RangeTokenNode[], ChangeData | null]
  >(
    ([oldEdits, newEdits, previousChange], currentChange) => {
      if (
        !previousChange ||
        !isDelete(previousChange) ||
        !isInsert(currentChange)
      ) {
        return [oldEdits, newEdits, currentChange];
      }

      const [oldDiffs, newDiffs] = diffText(
...
export default function markEdits(
  hunks: HunkData[],
  { type = "block" }: MarkEditsOptions = {}
): TokenizeEnhancer {
